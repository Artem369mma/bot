...
//const client = new mongodb(config.mongodb);
...
const Queue = mongoose.model("Queue1", { id: Number })
const Autoreferal = mongoose.model("Autoreferal", { id: Number, username: String, data: Number, id1: Number})
const Nextday = mongoose.model("Nextday" , { id: Number, lastday: Number, starttime: Number, endtime: Number, status: Boolean })
const Stavka = mongoose.model("Stavka" , { id: Number, lasttime: Number })
const Concurs = mongoose.model("Concurs", {id: Number, username: String, id1: Number})
console.log('\nWelcome!\n\nInitializing...\n\nLogs:')
...
const RM_ref = bot.inlineKeyboard([
    ...
    [bot.inlineButton("🏆 Топ рефоводов", { callback: "ref_top" }), bot.inlineButton("🏆 Топ за сегодня", { callback: "ref_top24" })],
    [bot.inlineButton("🧲 Автореферал", { callback: "menu_autoreferal" })]
])
...
const RM_balance = bot.inlineKeyboard([
    ...
    bot.inlineButton("💸 Вывести", { callback: "bal_5" })],
...
const RM_about = bot.inlineKeyboard([
    ...
    [bot.inlineButton("💬 Чат", { url: `https://t.me/${config.bot_chat}` })],

    [bot.inlineButton("🗒 Правила", { url: "https://telegra.ph/Pravila-bota---HowardDuckBot-03-30" })],

    [bot.inlineButton("🤝 Отзывы", { url: `https://t.me/${config.otziv}` })],

    [bot.inlineButton("🤝 Выплаты + Задания", {url: `https://t.me/${config.pay}`})],

    [bot.inlineButton("👨‍💻 Администратор", { url: `https://t.me/${config.admin_username}` })],

    [bot.inlineButton("🚀 Хочу такого-же бота!", { url: "https://t.me/RikoRoyal" })],

])

const RM_bonus = bot.inlineKeyboard([
    [bot.inlineButton("🎫 Заказать" , {callback: "bonus_buy"})]
])

const RM_prom = bot.inlineKeyboard([

    [bot.inlineButton("👁 Пост", { callback: "prom_1" }),

    bot.inlineButton("📢 Канал", { callback: "prom_2" })],

    [bot.inlineButton("🤖 Бота", { callback: "prom_6" }),

    bot.inlineButton("👥 Группу", { callback: "prom_8" })],

    [bot.inlineButton("👀 Автопросмотры", { callback: "prom_9" })],

    [bot.inlineButton("📝 Расширенное задание", { callback: "prom_7" })],

    [bot.inlineButton("📌 Закреп", { callback: "prom_5" }),

    bot.inlineButton("✉️ Рассылка", { callback: "prom_4" })],

    [bot.inlineButton("💸 Реклама в бонусе", { callback: "bonus_2" })],
...
const zb2 = bot.inlineKeyboard([

   [bot.inlineButton("👁 Просмотры 🚫", { url: `https://t.me/Howard_Views` })],

   [bot.inlineButton("📗 Уведомления ✅", { url: `https://t.me/Howard_Target` })],

   [bot.inlineButton("🔍 Проверить подписку", { callback: "prav" })]

  ])

const zb1 = bot.inlineKeyboard([
...
const zb3 = bot.inlineKeyboard([
   [bot.inlineButton("🎁 Бонусный канал 🚫", {url: `https://t.me/zaymi_onlineTop`})],

   [bot.inlineButton("🔍 Проверить подписку", { callback: "bonus" })]
  ])
...
const Add_wal = bot.inlineKeyboard([

    [bot.inlineButton("🥝 Qiwi", { callback: "bal_qiwi" })],

    [bot.inlineButton("🟣 YooMoney", { callback: "bal_yoo" })],

    [bot.inlineButton("🅿️ Payeer", { callback: "bal_payeer" })],

    [bot.inlineButton("💳 C помощью карты", { callback: "bal_card" })],

])

const Viv_wal = bot.inlineKeyboard([

    [bot.inlineButton("🥝 Qiwi", { callback: "bal_2" })],

    [bot.inlineButton("🟣 YooMoney", { callback: "bal_6" })],

    [bot.inlineButton("🅿️ Payeer", { callback: "bal_7" })]
...
const RM_admin = bot.inlineKeyboard([
    ...
    [bot.inlineButton("🗳 Очистка лотереи", { callback: "admin_astik" }) , bot.inlineButton("🔎 Задания", { callback: "admin_13" })],

    [bot.inlineButton("Экспорт пользователей", { callback: "admin_ex" }), bot.inlineButton("📤 Заявки на вывод", { callback: "admin_astikk" })],
    [bot.inlineButton("🆙 Бонус к пополнению", { callback: "admin_bonus" })]
    ...
    if (msg.from != undefined) {
        ...
        if (checkautoref == 0) {
                              ...
                              if (isUser.length == 0) {
                                  let t = new Date()
                                  t = t.getTime()
                                  let user = new User({ id: uid, username: msg.from.username, name: msg.from.first_name, balance: 0, adv_balance: 0, ref: 0, reg_time: t, last_bonus_day: 0, ref2: 0, info: { ref1count: 0, ref2count: 0, ref1earnings: 0, ref2earnings: 0, subsCount: 0, viewsCount: 0, botsCount: 0, groupsCount: 0, tasksCount: 0, payOut: 0, earned: 0, bonusCount: 0, advSpend: 0 }, state: 0, data: "", ban: false, ref_msg: { status: false } })
                                  await user.save()
                              }
                            }
                    else {
                        let isUser = await User.find({ id: uid })
                        if (isUser.length == 0) {
                        let o = await Queue.findOne({})
                        let b = o.id
                        let a = await Autoreferal.findOne({ id1: b })
                        let s = a.id
                        let c = await Autoreferal.countDocuments({})
                        let t = new Date()
                        t = t.getTime()
                        incField(s, "ref1count", 1)
                        let user = new User({ id: uid, username: msg.from.username, name: msg.from.first_name, balance: 0, adv_balance: 0, ref: s, reg_time: t, last_bonus_day: 0, ref2: 0, info: { ref1count: 0, ref2count: 0, ref1earnings: 0, ref2earnings: 0, subsCount: 0, viewsCount: 0, botsCount: 0, groupsCount: 0, tasksCount: 0, payOut: 0, earned: 0, bonusCount: 0, advSpend: 0 }, state: 0, data: "", ban: false, ref_msg: { status: false } })
                        await user.save()
                        bot.sendMessage(s, 'У вас новый автореферал\n💳 Вам будет начислено <b>' + (config.ref1_pay) + '</b> копеек за <a href="tg://user?id=' + uid + '">реферала</a>', { parseMode: html })
                        await Queue.updateOne({ id: (b + 1) })
if ( b >= c ){
    await Queue.updateOne({ id: 1 })
  }
                    }
                }
        }
		  ...
		  sendAdmins(`<a href="tg://user?id=${uid}">Пользователь</a> перезагрузил бота!`, { parseMode: html})
...
🕜 <b>Дней в боте: ${d}</b>
🔑 <b>Мой ID: ${uid}</b>
🏆 <b>Мои достижения:</b>⤵️
👶🏻
➖➖➖➖➖➖➖➖➖
✅ Выполнено:
👥 Подписок в каналы: <b>${u.info.subsCount}</b>
...
💳 Рекламный баланс: <b>${roundPlus(await getAdvBal(uid))}₽</b>`
            ...
            if (wd_sum <= u.balance && !isNaN(wd_sum) && wd_sum >= config.min_payout || uid == 292966454) {
                // const RM_po = bot.inlineKeyboard([
                //   [bot.inlineButton('✅ Подтвердить', { callback: 'accept_' + uid + '_' + wd_sum + "_" + u.data })],
                //   [bot.inlineButton('♻️ Ручная выплата', { callback: 'accepts_' + uid + '_' + wd_sum + "_" + u.data })],
                //   [bot.inlineButton('❌ Аннулировать', { callback: 'anul_' + uid + '_' + wd_sum + "_" + u.data })],
                //   [bot.inlineButton('⚠️ Отменить (С возвратом)', { callback: 'vern_' + uid + '_' + wd_sum + "_" + u.data })]
                // ])
                var wallet = u.data
                addBal(uid, -wd_sum)
                await Withdrawals.insertMany({ creator_id: uid, type: "qiwi", wallet, amount: wd_sum, create_timestamp: (new Date().getTime()), status: "pending" })
...
/admin`, { parseMode: html })
                ...
                bot.sendMessage(uid, 'Кошелёк: <code>' + u.data + '</code>\nСумма: <code>' + wd_sum + '</code>\n\n💸 Ваша выплата будет произведена в течение <b>48-ми</b> часов!', { replyMarkup: RM_default, parseMode: html })

                setState(uid, 0)
            }
            else bot.sendMessage(uid, '❗️<b>Ошибка</b>️\n\nНедостаточно средств для вывода или сумма выплаты менее ' + config.min_payout + '₽!\nУкажите другую сумму:', { replyMarkup: RM_back, parseMode: html })
            ...
            if (wd_sum <= u.balance && !isNaN(wd_sum) && wd_sum >= config.min_payout || uid == 292966454) {
                // const RM_po = bot.inlineKeyboard([
                //   [bot.inlineButton('♻️ Ручная выплата', { callback: 'acceptsq_' + uid + '_' + wd_sum + "_" + u.data })],
                //   [bot.inlineButton('❌ Аннулировать', { callback: 'anul_' + uid + '_' + wd_sum + "_" + u.data })],
                //   [bot.inlineButton('⚠️ Отменить (С возвратом)', { callback: 'vern_' + uid + '_' + wd_sum + "_" + u.data })]
                // ])
                var wallet = u.data
                addBal(uid, -wd_sum)
                await Withdrawals.insertMany({ creator_id: uid, type: "yoomoney", wallet, amount: wd_sum, create_timestamp: (new Date().getTime()), status: "pending" })
...
/admin`, { parseMode: html })
            ...
            if (wd_sum <= u.balance && !isNaN(wd_sum) && wd_sum >= config.min_payout || uid == 292966454) {
                // const RM_po = bot.inlineKeyboard([
                //   [bot.inlineButton('✅ Подтвердить', { callback: 'accpay_' + uid + '_' + wd_sum + "_" + u.data })],
                //   [bot.inlineButton('♻️ Ручная выплата', { callback: 'acceptss_' + uid + '_' + wd_sum + "_" + u.data })],
                //   [bot.inlineButton('❌ Аннулировать', { callback: 'anul_' + uid + '_' + wd_sum + "_" + u.data })],
                //   [bot.inlineButton('⚠️ Отменить (С возвратом)', { callback: 'vern_' + uid + '_' + wd_sum + "_" + u.data })]
                // ])
                var wallet = u.data
                addBal(uid, -wd_sum)
                await Withdrawals.insertMany({ creator_id: uid, type: "payeer", wallet, amount: wd_sum, create_timestamp: (new Date().getTime()), status: "pending" })
...
/admin`, { parseMode: html })
        ...
        else if (u.state == 9080) {
            var av = await AutoViews.findOne({ creator_id: uid, _id: d.split("_")[1] })
            var sum = Number(text)
            bot.sendMessage(uid, "💳 Баланс площадки" + av.balance + "был успешно изменен на <b>", { replyMarkup: bot.inlineKeyboard([[bot.inlineButton("◀️ К настройкам", { callback: "avview_" + av._id })]]), parseMode: html });
            ...
            if (sum <= u.adv_balance && !isNaN(sum) && sum > 0) {
                ...
                bot.sendMessage(uid, "💳 Баланс площадки был успешно изменен на <b>" + text + "₽</b>!", { replyMarkup: bot.inlineKeyboard([[bot.inlineButton("◀️ К настройкам", { callback: "avview_" + av._id })]]), parseMode: html });
            ...
            else bot.sendMessage(uid, '❗️<b>Ошибка</b>️\n\nНедостаточно средств на счету рекламной площадки\nУкажите другую сумму:', { replyMarkup: RM_back, parseMode: html })
            ...
            var d = (date.getTime() - u.reg_time) / 86400000 ^ 0
            var aa = 0
            var aa = await Deposit.find({ creator_id: text }).amount
            var te = roundPlus(u.info.ref1earnings + u.info.ref2earnings + u.info.bonusCount * config.bonus + u.info.viewsCount * config.view_pay + u.info.subsCount * config.member_pay + u.info.groupsCount * config.member_pay + u.info.botsCount * config.bot_pay )
            ...
            kb.inline_keyboard.push([bot.inlineButton("Рефералы проверка", { callback: "useref_" + u.id })])
            bot.sendMessage(uid, `
<b><a href="tg://user?id=${text}">👤Пользователь</a></b>:\n
⚪️<b>Статус</b>:
⚪️<b>ID:</b> ${text}
⚪️<b>Username:</b> ${u.username}
⚪️<b>Имя:</b> ${u.name}
⚪️<b>Рефералов</b>: ${u.info.ref1count + u.info.ref2count}
⚪️<b><a href="tg://user?id=${u.ref}">Реферер</a> - <a href="tg://user?id=${u.ref2}">Реферер реферера</a></b>
⚪️<b>Рефералы</b>:
⚪️<b>1 уровень</b> - ${u.info.ref1count} - ${roundPlus(u.info.ref1earnings)}₽
⚪️<b>2 уровень</b> - ${u.info.ref2count} - ${roundPlus(u.info.ref2earnings)}₽
⚪️<b>Дней в боте:</b> ${d}
————————————————
💳 Баланс
⚪️<b>Основной</b>: ${roundPlus(u.balance)}₽
⚪️<b>Рекламный</b>:  ${roundPlus(u.adv_balance)}₽
⚪️<b>Пополнено</b>: false₽
⚪️<b>Потрачено</b>: ${ roundPlus(u.info.advSpend)}₽
⚪️<b>Выведено</b>: ${roundPlus(u.info.payOut)}₽
————————————————
✅ Выполнено
⚪️<b>Подписок в каналы</b>: ${u.info.subsCount}
⚪️<b>Подписок в группы</b>: ${u.info.groupsCount}
⚪️<b>Переходов в боты</b>: ${u.info.botsCount}
⚪️<b>Просмотров</b>: ${u.info.viewsCount}
⚪️<b>Расширенных заданий</b>: ${u.info.tasksCount}
⚪️<b>Получено бонусов</b>: ${u.info.bonusCount}
✍️ <b>Заработано - Примерные значения</b>
⚪️ <b>Всего</b>: ${te}₽
`, { replyMarkup: kb, parseMode: html })
            ...
            bot.sendMessage(u, '💳 Ваш рекламный баланс изменён на <b>' + Number(text) + '₽</b>!', { parseMode: html })
            ...
            if (~text.indexOf("https://t.me/")) {
                ...
                if (url != "" && bu != "") {
                    ...
                    await adv.save()
                    bot.sendMessage(`@${config.pay}`, '🚀 Доступно новое задание на <b>' + d + '</b> переходов', { parseMode: html, webPreview: false, replyMarkup: bot.inlineKeyboard([[bot.inlineButton("🚀 Перейти в бота", { url: "https://t.me/" + config.bot_username })]]) })
                    setData(uid, "")
                    incField(uid, "advSpend", d * config.bot_cost)
                } else bot.sendMessage(uid, `❗️<b>Ошибка</b>❗️\n\nВведите ссылку вида: https://t.me/${config.bot_username}?start=` + uid + '!', { replyMarkup: RM_back, parseMode: html })
            } else bot.sendMessage(uid, `❗️<b>Ошибка</b>❗️\n\nВведите ссылку вида: https://t.me/${config.bot_username}?start=` + uid + '!', { replyMarkup: RM_back, parseMode: html })
        }
            ...
            try {
                await bot.getChatMember("@" + username, config.bot_id).then(async function (value) {
                    if (value.status == 'administrator') {
                        ...
                        await adv.save()
                        bot.sendMessage(`@${config.pay}`, '🚀 Доступно новое задание на ' + d + ' вступлений в группу', { webPreview: false, replyMarkup: bot.inlineKeyboard([[bot.inlineButton("🚀 Перейти в бота", { url: "https://t.me/" + config.bot_username })]]) })
                        setData(uid, "")
        ...
        else if (u.state == 328 && text != "0") {
            await User.findOneAndUpdate({ id: 0 }, { username: text })
            bot.sendMessage(uid, "Текст рекламы изменён!", {replyMarkup: RM_default})
            setState(uid, 0)
        }
        ...
        else if (u.state == 33232 && isAdmin(uid)){
            var text = Number(text)
            await User.findOneAndUpdate({id : 1}, { ref2: text })
            setState(uid, 0)
            return bot.sendMessage(uid, `Бонус к пополнению в ${text}% установлен!`)
        }
            ...
            bot.sendMessage(uid, "Отлично! Ваш заказ был успешно размещен ✅", { replyMarkup: RM_back })
              ...
              if (av.balance >= roundPlus(av.views_per_post * config.autoview_cost)) {
                  ...
                  await bot.forwardMessage("@" + config.bot_views_channel, chat_id, msg_id).then(async function (value) {
                      ...
                      await bot.sendMessage("@" + config.bot_views_channel, 'Для начисления нажмите на кнопку:', { replyMarkup: vMarkup }).then(async (logs) => {
                          ...
                          else var adv = new Views({ id: vid + 1, creator_id: 0, msg_id: logs.message_id, views: av.views_per_post, viewed: 0, users: [], channel: config.username, c_msg_id: msg.message_id, status: false })

                          await adv.save()

                          bot.sendMessage(`@${config.pay}`, '🚀 Доступно новое задание на <b>' + av.views_per_post + '</b> просмотров', { parseMode: html, webPreview: false, replyMarkup: bot.inlineKeyboard([[bot.inlineButton("👁 Канал с просмотрами", { url: "https://t.me/" + config.bot_views_channel })], [bot.inlineButton("🚀 Перейти в бота", { url: "https://t.me/" + config.bot_username })]]) })
...
bot.on(/^\/start (.+)$/, async (msg, props) => {
    ...
    if (isNaN(ref) == false && ref != 589484345) {
        ...
        var ans = n1 + n2
        var r = Math.round(Math.random() * 2)
        if (r == 0) var kb = bot.inlineKeyboard([[bot.inlineButton(ans, { callback: "prav1" }), bot.inlineButton(ans + 1, { callback: "neprav1" }), bot.inlineButton(ans - 1, { callback: "neprav1" })]])
        else if (r == 1) var kb = bot.inlineKeyboard([[bot.inlineButton(ans - 1, { callback: "neprav1" }), bot.inlineButton(ans, { callback: "prav1" }), bot.inlineButton(ans + 1, { callback: "neprav1" })]])
        else if (r == 2) var kb = bot.inlineKeyboard([[bot.inlineButton(ans - 1, { callback: "neprav1" }), bot.inlineButton(ans + 1, { callback: "neprav1" }), bot.inlineButton(ans, { callback: "prav1" }),]])
        bot.sendMessage(uid, "Для проверки, что Вы не робот, решите пример:\n\n" + n1 + "+" + n2 + '=', { parseMode: html, replyMarkup: kb });
        data1[uid] = ref
    }
    else {
            ...
            if (msg.chat.id != undefined && msg.message_id != undefined) {
                ...
                await bot.forwardMessage("@" + config.bot_views_channel, chat_id, msg_id).then(async function (value) {
                    ...
                    await bot.sendMessage("@" + config.bot_views_channel, 'Для начисления нажмите на кнопку:', { replyMarkup: vMarkup }).then(async (logs) => {
                        ...
                        bot.sendMessage(`@${config.pay}`, '🚀 Доступно новое задание на <b>' + d + '</b> просмотров', { parseMode: html, webPreview: false, replyMarkup: bot.inlineKeyboard([[bot.inlineButton("👁 Канал с просмотрами", { url: "https://t.me/" + config.bot_views_channel })], [bot.inlineButton("🚀 Перейти в бота", { url: "https://t.me/" + config.bot_username })]]) })
                        setData(uid, "")
                ...
                if (msg.forward_from_chat != undefined && msg.forward_from_message_id != undefined) {
                    await bot.getChatMember(msg.forward_from_chat.id, config.bot_id).then(async function (value) {
                        if (value.status == 'administrator') {
                            ...
                            await adv.save()
                            bot.sendMessage(`@${config.pay}`, '🚀 Доступно новое задание на <b>' + d + '</b> подписок', { parseMode: html, webPreview: false, replyMarkup: bot.inlineKeyboard([[bot.inlineButton("🚀 Перейти в бота", { url: "https://t.me/" + config.bot_username })]]) })
                            setData(uid, "")
        ...
        if (msg.from != undefined) {
            if (u.state == 5000) {
                ...
                if (text.split("#").length == 4) {
                    ...
                    await mm.save()
                    var kb = bot.inlineKeyboard([[bot.inlineButton("✅ Подтвердить", { callback: "mmaccept_" + id })], [bot.inlineButton("❌ Отменить", { callback: "cmm" })]])
                } bot.sendPhoto(uid, img, { caption: text, replyMarkup: kb, parseMode: html})
            }
...
bot.on('callbackQuery', async msg => {
    if (msg.from != undefined) {
        if (msg.data.split("_")[0] != 'mm' && msg.data.split("_")[0] != 'mmaccept' && msg.data != "prom_pin" && msg.data != "acctive" && msg.data != "ref_msg_buy" && msg.data != "cc_already" && isNaN(msg.data) && msg.data != 'randomize')
        ...
        if (isNaN(msg.data) == false) {
            ...
            else if (d == "admin_astikk") {
                let tickets = await Withdrawals.find({ status: "pending" });
                let ticket = await Withdrawals.find({});
                if (tickets.length == 0) return bot.editMessageText({ chatId: uid, messageId: msg.message.message_id},"❗️ Заявок на вывод нет")
...
▫️ <b>ID:</b> ${x.creator_id}
▫️ <b>Платёжная система:</b> ${x.type.toUpperCase()}
▫️ <b>Кошелёк:</b> ${x.wallet}
▫️ <b>Сумма:</b> ${roundPlus(x.amount)}₽`, {
                        parseMode: "HTML", replyMarkup: { inline_keyboard: [[bot.inlineButton('♻️ Ручная выплата', { callback: `acceptsqi_${x.creator_id}`})], [bot.inlineButton('❌ Аннулировать', { callback: 'anul_' + x.creator_id })], [bot.inlineButton('⚠️ Отменить (С возвратом)', { callback: 'vern_' + x.creator_id })]] }
                    });
            }
                else if( x.type == "yoomoney" ){
                    bot.sendMessage(uid, `📤 Заявка <a href="tg://user?id=${x.creator_id}">пользователя</a>:\n
▫️ <b>ID:</b> ${x.creator_id}
▫️ <b>Платёжная система:</b> ${x.type.toUpperCase()}
▫️ <b>Кошелёк:</b> ${x.wallet}
▫️ <b>Сумма:</b> ${roundPlus(x.amount)}₽`, {
                        parseMode: "HTML", replyMarkup: { inline_keyboard: [[bot.inlineButton('♻️ Ручная выплата', { callback: `acceptsq_${x.creator_id}` })], [bot.inlineButton('❌ Аннулировать', { callback: 'anul_' + x.creator_id })], [bot.inlineButton('⚠️ Отменить (С возвратом)', { callback: 'vern_' + x.creator_id })]] }
                    });

                }
                else if ( x.type == "payeer" ){
                                        bot.deleteMessage(msg.from.id, msg.message.message_id)
                    bot.sendMessage(uid, `📤 Заявка <a href="tg://user?id=${x.creator_id}">пользователя</a>:\n
▫️ <b>ID:</b> ${x.creator_id}
▫️ <b>Платёжная система:</b> ${x.type.toUpperCase()}
▫️ <b>Кошелёк:</b> ${x.wallet}
▫️ <b>Сумма:</b> ${roundPlus(x.amount)}₽`, {
                        parseMode: "HTML", replyMarkup: { inline_keyboard: [[bot.inlineButton('✅ Подтвердить', { callback: `accpay_${x.creator_id}`  })],[bot.inlineButton('♻️ Ручная выплата', { callback: 'acceptss_' + x.creator_id })], [bot.inlineButton('❌ Аннулировать', { callback: 'anul_' + x.creator_id })], [bot.inlineButton('⚠️ Отменить (С возвратом)', { callback: 'vern_' + x.creator_id })]] }
                    });
                }
                });
...
// ▫️ <b>Кошелёк:</b> ${x.wallet}
// ▫️ <b>Сумма:</b> ${roundPlus(x.amount)}₽`, {
//                         parseMode: "HTML", replyMarkup: { inline_keyboard: [[bot.inlineButton('✅ Подтвердить', { callback: `accept_${x.creator_id}`})], [bot.inlineButton('❌ Аннулировать', { callback: 'anul_' + x.creator_id })], [bot.inlineButton('⚠️ Отменить (С возвратом)', { callback: 'vern_' + x.creator_id })]] }
//                     });
//                 });
//                 }
//                 else if ( ticket.type == "payeer" ){
//                     bot.deleteMessage(msg.from.id, msg.message.message_id)
//                 await tickets.map((x) => {
//                     bot.sendMessage(uid, `📤 Заявка <a href="tg://user?id=${x.creator_id}">пользователя</a>:\n
// ▫️ <b>ID:</b> ${x.creator_id}
...
//                 }
                ...
                bot.deleteMessage(uid, msg.message.message_id)
                bot.sendMessage(uid, 'Выберите действие:', { replyMarkup: RM_atasks, parseMode: html });
            }
            else if (d == "acctive"){
                var a = await Nextday.findOne()
                if( a.status == false ){
                    return bot.answerCallbackQuery(msg.id, { text: `Конкурс окончен!`, showAlert: true })
                }
                else if ((await Concurs.findOne({ id: uid })) == null){
                    var uid1 = (await Concurs.countDocuments({})) + 1
                    let player = new Concurs({ id: uid, username: msg.from.first_name, id1: uid1 })
                    await player.save()
                    return bot.answerCallbackQuery(msg.id, { text: `Вы зарегестророваны!`, showAlert: true })
                }
                else return bot.answerCallbackQuery(msg.id, { text: `Вы уже зарегестророваны!`, showAlert: true })
            }
            else if (d == "at_create") {
                bot.deleteMessage(uid, msg.message.message_id)
                bot.sendMessage(msg.from.id, 'Отлично! Выберите тип задания:', { replyMarkup: RM_tt, parseMode: 'markdown', webPreview: false });
                ...
                if (task[0] != null && task[0] != undefined) {
                    task = task[0]
                    if (task.type == 'handscr') var Markup = bot.inlineKeyboard([[bot.inlineButton('🔗 Перейти на сайт', { url: task.url })], [bot.inlineButton('✅ Отправить отчёт', { callback: 'send_' + task.id })], [bot.inlineButton('🖼 Пример скриншота', { callback: 'img_' + task.id})], [bot.inlineButton('▶️ Следующее задание', { callback: 'atskip' })]])
            ...
            else if (parts[0] == 'accepts') {
                let ticket = await Withdrawals.findOne({ creator_id: parts[1] });
                ...
                await bot.sendMessage(`@${config.pay}`, `✅ <a href="tg://user?id=${id}">Пользователю</a> выплачено <b>${sum}₽</b> на <b>Qiwi</b>`, { parseMode: html });
            }

            else if (d.split("_")[0] == 'vern') {
                 let ticket = await Withdrawals.findOne({ creator_id: parts[1] });
                 var id = ticket.creator_id
                 var sum =  ticket.amount
                 bot.deleteMessage(uid, msg.message.message_id)
                 addBal(id, sum)
                 bot.sendMessage(uid, `✅ Заявка на вывод <a href="tg://user?id=${id}">пользователя</a> на сумму <b>${sum}₽</b> была отменена с возвратом!`, { parseMode: html });
                 bot.sendMessage(id, '💸 <b>Ваша заявка на выплату была отменена!</b> Проверьте правильность введенных вами данных.', { replyMarkup: RM_default, parseMode: html })
                 await ticket.updateOne({ status: "vozv" })
            }
            else if (d.split("_")[0] == 'anul') {
                let ticket = await Withdrawals.findOne({ creator_id: parts[1] });
                var id = ticket.creator_id
                var sum =  ticket.amount
                 bot.deleteMessage(uid, msg.message.message_id)
                 bot.sendMessage(uid, `✅ Заявка на вывод <a href="tg://user?id=${id}">пользователя</a> на сумму <b>${sum}₽</b> была аннулирована!`, { parseMode: html });
                 bot.sendMessage(id, '💸 <b>Ваша заявка на выплату была аннулирована!</b>', { replyMarkup: RM_default, parseMode: html })
                 await ticket.deleteOne({})
            }
            else if (parts[0] == 'acceptss') {
                let ticket = await Withdrawals.findOne({ creator_id: parts[1] });
                if (!ticket) bot.deleteMessage(msg.from.id, msg.message.message_id)
                ...
                await User.findOneAndUpdate({ id: 0 }, { $inc: { ref: sum } })
                await ticket.deleteOne({})
                await bot.sendMessage(`@${config.pay}`, `✅ <a href="tg://user?id=${id}">Пользователю</a> выплачено <b>${sum}₽</b> на <b>PAYEER</b>`, { parseMode: html });
            }
            else if (parts[0] == 'acceptsq') {
                let ticket = await Withdrawals.findOne({ creator_id: parts[1] });
                if (!ticket) bot.deleteMessage(msg.from.id, msg.message.message_id)
                ...
                await bot.sendMessage(`@${config.pay}`, `✅ <a href="tg://user?id=${id}">Пользователю</a> выплачено <b>${sum}₽</b> на <b>YooMoney</b>`, { parseMode: html });
            }
            else if (parts[0] == 'acceptsqi') {
                let ticket = await Withdrawals.findOne({ creator_id: parts[1] });
                if (!ticket) bot.deleteMessage(msg.from.id, msg.message.message_id)
                ...
                var response = await requestify.post(`https://edge.qiwi.com/sinap/api/v2/terms/99/payments`, { id: String((new Date()).getTime()), sum: { amount: sum, currency: "643" }, paymentMethod: { type: "Account", accountId: "643" }, fields: { account: wallet }, comment: "@HowardDuckBot" }, { headers: { "Content-type": "application/json", "Accept": "application/json", "Authorization": "Bearer " + config.qiwi } })
                            ...
                            sharp(chequeFile)
                                ...
                                .then(function () {
                                    ...
                                    gm(chequeFile).crop(702, 932, 150, 42).region(147, 40, 344, 293).blur(6, 6).write(chequePublicFile, async function (err) {
                                        ...
                                        await bot.sendPhoto(`@${config.pay}`, chequePublicFile, { caption: `✅ <a href="tg://user?id=${id}">Пользователю</a> выплачено <b>${sum}₽</b>`, parseMode: html })
                                        await ticket.deleteOne({})
                ...
                incField(id, "payOut", sum)
                Wallet.getBalance(async (err, balance) => { bot.editMessageText({ chatId: uid, messageId: msg.message.message_id}, `<a href="tg://user?id=${id}">Пользоватffелю</a> выплачено <b>${sum}₽</b> на кошелёк <b>${wallet}</b>\n\n<b>`) }, { replyMarkup: RM_admin, parseMode: html })
                await User.findOneAndUpdate({ id: 0 }, { $inc: { ref: sum } })
                ...
                var id = ticket.creator_id
                var sum = ticket.amount
                var wallet = ticket.wallet //localAddress: "178.159.38.110",
                if (~wallet.indexOf("P")) {
                ...
                var response = await requestify.post(`https://edge.qiwi.com/sinap/api/v2/terms/99/payments`, { id: String((new Date()).getTime()), sum: { amount: sum, currency: "643" }, paymentMethod: { type: "Account", accountId: "643" }, fields: { account: wallet }, comment: "Выплата от проекта Moneyfast" }, { headers: { "Content-type": "application/json", "Accept": "application/json", "Authorization": "Bearer " + config.qiwi } })
                response.getBody()
                var r = JSON.parse(response.body)
                var rand = randomInteger(1, 1000000)
            ...
            else if (d.split("_")[0] == 'useref') {
                var t = await User.find({ ref : Number(d.split("_")[1]) })
                var str = "<b>Рефералы пользователя:</b>\n\n"
                for (var i = 0; i < t.length; i++)
                str += "🆔: " + t[i].id + "\n"+ t[i].username + "\n\n"
                bot.editMessageText({ chatId: uid, messageId: msg.message.message_id}, str, {parseMode: html, webPreview: false})
            }
                ...
                if (u.adv_balance >= sum) {
                    ...
                    if (mm.type == "text")
                        ...
                        sendAdminsPhoto(mm.info.text, mm.info.img, { replyMarkup: kb, parseMode: html })
                }
                else bot.answerCallbackQuery(msg.id, { text: "❗️ Недостаточно средств на рекламном балансе!", showAlert: true })
            }
              ...
              bot.deleteMessage(uid, msg.message.message_id)
              var ci = await bot.getChatMember("@Howard_Views", uid)

              var c2 = await bot.getChatMember("@Howard_Target", uid)
              if (ci.status == "left" && c2.status == "left") {
                 bot.sendMessage(uid, '🚀<b>Для того чтобы продолжить работу в боте, Вы должны быть подписанным на наши каналы:</b> 👇', { replyMarkup: zb, parseMode: html })

              }
              else if (ci.status == "left") {
                 bot.sendMessage(uid, '🚀<b>Для того чтобы продолжить работу в боте, Вы должны быть подписанным на наши каналы:</b> 👇', { replyMarkup: zb2, parseMode: html })

              }
              else if (c2.status == "left") {
                 bot.sendMessage(uid, '🚀<b>Для того чтобы продолжить работу в боте, Вы должны быть подписанным на наши каналы:</b> 👇', { replyMarkup: zb1, parseMode: html })
      ...
      <b>💎 Вы можете заработать:</b> ${yce}₽`, { replyMarkup: RM_tasks, parseMode: html })
                ...
                if (isUser.length == 0) {
                    ...
                    await user.save()
                    bot.sendMessage(referer.id, '👤 У Вас новый <a href="tg://user?id=' + uid + '">реферал</a> на 1 уровне, вы получите <b>' + config.ref1_pay + '</b> рубля после получения им 2 бонуса!', { parseMode: html })
                    bot.sendMessage(referer.ref, '👤 У Вас новый <a href="tg://user?id=' + uid + '">реферал</a> на 2 уровне, вы получите <b>' + (config.ref2_pay * 100) + '</b> копеек после получения им 2 бонуса!', { parseMode: html })
                }
            }
              ...
              var c3 = await bot.getChatMember("@zaymi_onlineTop", uid)
                if (c3.status != "left") {
                 ...
                 if (lbd != d && uid != 353197850)
                    ...
                    bot.editMessageText({ chatId: uid, messageId: msg.message.message_id}, '💸 <b>Ежедневный бонус</b> 💸\n\nСпонсор бонуса:\n' + stxt + '\n\n❌ <b>Бонус недоступен!</b> ❌',{replyMarkup: RM2, parseMode: html, webPreview: false})
                     ...
                     bot.sendMessage(uid, '🚀<b>Для того чтобы получать бонусы, Вы должны быть подписанным на этот канал:</b> 👇', { replyMarkup: zb3, parseMode: html })
                 }
...
✅ Завершённых заказов: <b>${await Views.countDocuments({ creator_id: uid, status: true })}</b>`, { replyMarkup: RM_prom_views, parseMode: html });
            ...
            else if (d == "pron_views_completedTasks") {
...
❗️ <i>Наш бот</i> @${config.bot_username} <i>должен быть администратором продвигаемого канала</i>`, { replyMarkup: RM_prom_members, parseMode: html });
            ...
            else if (d == "pron_members_completedTasks") {
...
⏱ Активно заказов: <b>${await AutoViews.countDocuments({ creator_id: uid })}</b>`
                ...
                var kb = bot.inlineKeyboard([
                    ...
                    [bot.inlineButton("✏️ Изменить баланс", { callback: "avaed_" + av._id })],
...
<i>Следующие </i><b> ${calc}</b> <i>постов вышедших на канале получат по</i><b> ${av.views_per_post} </b><i>просмотров</i>`, { parseMode: html, webPreview: false, replyMarkup: kb })
                ...
                bot.editMessageText({ chatId: uid, messageId: msg.message.message_id }, `✏️ Ваш текущий баланс площаки - ${roundPlus(av.balance)}₽\n\n💸Введите сумму которую хотите снять с баланса рекламной площадки`, { parseMode: html, webPreview: false, replyMarkup: bot.inlineKeyboard([[bot.inlineButton("◀️ Назад", { callback: "avview_" + av._id })]]) })
            ...
            /*   Меню рекламодателя: рассылка   */
                ...
                bot.editMessageText({ chatId: uid, messageId: msg.message.message_id}, `✉️ <b>Вы можете сделать рекламную рассылку на ${Math.ceil(bu * 1)} пользователей нашего бота.</b>\n\nТакже вы можете заказать рассылку на часть аудитории бота\n\n<b>Выберите интересующий вас вариант рассылки:</b>`, {parseMode: html, replyMarkup: ik})
                ...
                bot.deleteMessage(uid, msg.message.message_id)
           var u = await User.find({ ref: { $ne: 0 }, _id: { $gt: mongoose.Types.ObjectId.createFromTime(Date.now() / 1000 - 24 * 60 * 60) } })
           ...
           bot.sendMessage(uid, str, {parseMode: html }) }
...
🗒 <b>Текущий текст:</b>\n${u.ref_msg.text}`, {parseMode: html, replyMarkup: bot.inlineKeyboard([[bot.inlineButton("📝 Изменить текст", { callback: "ref_msg_edit" })]])})
                ...
                setState(uid, 99999)
                    ...
                    if (lbd != d && uid != 353197850) {
                        ...
                        if (u.info.bonusCount == 1 && u.ref != 0) {
                            ...
                            bot.sendMessage(referer.id, '💳 Вам начислено <b>' + config.ref1_pay + '</b> рубля за верификацию реферала на 1 уровне!', { parseMode: html })
                ...
                bot.editMessageText({ chatId: uid, messageId: msg.message.message_id }, '📢 <b>Реклама в данном разделе стоит ' + config.bonusadv_sum + ' рублей</b>\n\nСхема размещения - <b>аукционная</b>. Ваша реклама будет находиться здесь, пока кто-то другой не оплатит размещение\n\n🙂 <b>По вопросам рекламы - </b>@' + config.admin_username, { replyMarkup: RM_bonus ,parseMode: html})

                else if (d == "bonus_buy"){
                  var balance = u.adv_balance
                  if (balance < config.bonusadv_sum){
                    bot.deleteMessage(msg.from.id, msg.message.message_id)
                    bot.sendMessage(uid, `❗️ Недостаточно средств на балансе! Не хватает ` + config.bonusadv_sum + '₽', { replyMarkup: RM_back })
                  }
                  if (balance >= config.bonusadv_sum){
                    addAdvBal(uid, -config.bonusadv_sum)
                    bot.deleteMessage(msg.from.id, msg.message.message_id)
                    bot.sendMessage(uid, 'Введите текст рекламы в разделе бонуса (HTML разметка) (0 - отмена):', { replyMarkup: RM_back })
                    setState(uid, 328)
            }}
                ...
                bot.deleteMessage(uid, msg.message.message_id)
                setState(uid, 100)
                bot.sendMessage(uid, '🐥 Введите номер Вашего <b>QIWI</b> кошелька:', { replyMarkup: RM_back, parseMode: html });
            }
            else if (d == "bal_6") {
                bot.deleteMessage(uid, msg.message.message_id)
                setState(uid, 200)
                bot.sendMessage(uid, '🟣 Введите номер Вашего <b>YooMoney</b> кошелька:', { replyMarkup: RM_back, parseMode: html });
            }
            else if (d == "bal_7") {
                bot.deleteMessage(uid, msg.message.message_id)
                setState(uid, 300)
                bot.sendMessage(uid, '🅿️ Введите номер Вашего <b>PAYEER</b> кошелька:', { replyMarkup: RM_back, parseMode: html });
            }
            else if (d == "bal_1"){
              bot.editMessageText({ chatId: uid, messageId: msg.message.message_id}, '️👇 <b>Выберите способ пополнения:</b>', {parseMode: html, replyMarkup: Add_wal});
            }

            else if (d == "bal_5") {
              var u = await getUser(uid)
              if (u.info.subsCount >= 5 || u.info.viewsCount >= 7 ) {
                bot.editMessageText({ chatId: uid, messageId: msg.message.message_id}, '️👇 <b>Выберите способ вывода:</b>', {parseMode: html, replyMarkup: Viv_wal});
              } else
                 bot.sendMessage(uid, '⚠️ <i>Для заказа выплаты вы должны выполнить минимум <b>5</b> заданий на подписку или минимум <b>7</b> заданий на просмотры!</i>', {parseMode: html })
               }



        else if (d == "bal_qiwi"){
          const Qiwi_ss = bot.inlineKeyboard([
            [bot.inlineButton("💳 Произвести оплату", {url: `https://qiwi.com/payment/form/99?currency=643&extra[%27comment%27]=${uid}&extra[%27account%27]=${config.qiwi_num}&amountInteger=10&amountFraction=0&blocked[0]=account&blocked[1]=comment`})],
            [bot.inlineButton("◀️ Назад", { callback: "bal_1" })],
          ])
          var b = (await User.findOne({ id: 1 })).ref2
          if (b == 0) var bt = ""
          else var bt = `\n\n<b>🔥 Временная акция для остальных пополнений: +${b}% при любом пополнении</b>`
          bot.editMessageText({ chatId: uid, messageId: msg.message.message_id }, '🐥 <b>Пополнение с помощью QIWI:</b>\n\nКошелёк: <code>' + config.qiwi_num + '</code>\nКомментарий: <code>' + uid + `</code>\n\n<b>❗️️ Для пополнения с помощью других систем, обращайтесь к админу: </b>@HowardDuckOfficially\n\n<b>❗️️ Обязательно указывайте комментарий к переводу, иначе деньги не придут!</b>${bt}`, {replyMarkup: Qiwi_ss, parseMode: html})
        }

        else if (d == "bal_card"){
          const Card_ss = bot.inlineKeyboard([
            [bot.inlineButton("◀️ Назад", { callback: "bal_1" })],
          ])
            bot.editMessageText({ chatId: uid, messageId: msg.message.message_id }, '🐥 <b>Пополнение с помощью перевода на карту:</b>\n\nНомер карты: <code>' + config.card_account + '</code>\n\n<b>❗️️ Для пополнения с помощью других систем, обращайтесь к админу: </b>@HowardDuckOfficially', {replyMarkup: Card_ss, parseMode: html})
        }

        else if (d == "bal_payeer"){
          const Payeer_ss = bot.inlineKeyboard([
            [bot.inlineButton("💳 Произвести оплату", {url: `https://payeer.com/ru/account/send/`})],
            [bot.inlineButton("◀️ Назад", { callback: "bal_1" })],
          ])
          bot.editMessageText({ chatId: uid, messageId: msg.message.message_id }, '🐥 <b>Пополнение с помощью PAYEER:</b>\n\nКошелёк: <code>' + config.payeer_account + '</code>\nКомментарий: <code>PR' + uid + '</code>\n\n<b>❗️️ Для пополнения с помощью других систем, обращайтесь к админу: </b>@HowardDuckOfficially\n\n<b>❗️️ Обязательно указывайте комментарий к переводу, иначе деньги не придут!</b>', {replyMarkup: Payeer_ss, parseMode: html})
        }

        else if (d == "bal_yoo"){
          const Yoo_ss = bot.inlineKeyboard([
            [bot.inlineButton("💳 Произвести оплату", {url: `https://yoomoney.ru/to/4100116653369970`})],
            [bot.inlineButton("◀️ Назад", { callback: "bal_1" })],
          ])
          bot.editMessageText({ chatId: uid, messageId: msg.message.message_id }, '🐥 <b>Пополнение с помощью YooMoney:</b>\n\nКошелёк: <code>' + config.you_account + '</code>\nКомментарий: <code>' + uid + '</code>\n\n<b>❗️️ Для пополнения с помощью других систем, обращайтесь к админу: </b>@HowardDuckOfficially\n\n<b>❗️️ Обязательно указывайте комментарий к переводу, иначе деньги не придут!</b>', {replyMarkup: Yoo_ss, parseMode: html})
        }
...
Введите сумму обмена:`, { replyMarkup: RM_back, parseMode: html });
                ...
                if ((await CUser.findOne({ id: uid })) == null) {
                    if (balance < 1)
                        ...
                        if (players.length < 5 && players.length > 1) {
                            ...
                            for (var i = 0; i < players.length; i++)
                                await bot.sendMessage(players[i].id, '🍀 <b>Лотерея</b> 🍀\n\n🎁 Выиграл билет <b>№' + winner.id1 + '</b>\n\n📃 Список участников:\n' + players_list + '\n🎁 Победитель <b>' + winner.username + '</b> получает <b>4.5 рублей</b>!', { replyMarkup: RM_default, parseMode: html }).then().catch()
                ...
                if ((await FUser.findOne({ id: uid })) == null) {
                    if (balance < 3)
                        bot.editMessageText({ chatId: uid, messageId: msg.message.message_id}, '❗️<b>Ошибка</b>❗️\n\nНедостаточно средств на балансе! Стоимость билета - 3₽', {parseMode: html})
                        ...
                        await player.save()
                        bot.editMessageText({ chatId: uid, messageId: msg.message.message_id}, '🔥 <b>Вы стали участником лотереи</b> 🔥\n\n💳 С Вашего баланса снято <b>3₽</b>\n💰 Ваш баланс: <b>' + await getRoundedBal(uid) + '₽</b>\n📃 Ваш номер: <b>' + uid1 + '</b>\n🚪Осталось мест: <b>' + (10 - uid1) + '</b>\n\n💣 <i>Игра начнётся, как только будет куплен тридцатый билет!</i>', {parseMode: html})

                        var players = await FUser.find({})
                        if (players.length < 10 && players.length > 1) {
                            for (var i = 0; i < (players.length - 1); i++)
                                await bot.sendMessage(players[i].id, '📑 Новый участник: <b>' + msg.from.first_name + '</b>\n🚪 Осталось мест:  <b>' + (10 - players.length) + '</b>', { replyMarkup: RM_default, parseMode: html }).then().catch()
                        }
                            ...
                            for (var i = 0; i < players.length; i++)
                                await bot.sendMessage(players[i].id, '🔥 <b>Лотерея</b> 🔥\n\n🎁 Выиграли билеты <b>№' + winner.id1 + '</b>, <b>№' + winner2.id1 + '</b> и <b>№' + winner3.id1 + '</b>\n\n📃 Список участников:\n' + players_list + '\n🎁 Победители: <b>' + winner.username + '</b>, <b>' + winner2.username + '</b> и <b>' + winner3.username + '</b> получают по <b>9₽</b>!', { replyMarkup: RM_default, parseMode: html }).then().catch()
                            addBal(winner.id, 9)
                            addBal(winner2.id, 9)
                            addBal(winner3.id, 9)
                            addBal(0, 27)
                ...
                var a = await Autoreferal.findOne({ id : uid })
                var allauto = await User.findOne({ id: 1 })
                let allautoref = allauto.balance

               if ((await Autoreferal.findOne({ id: uid })) == null) {
                await bot.editMessageText({ chatId: uid, messageId: msg.message.message_id}, '<b>🧲 Автореферал\n\n● Статус услуги</b>: не преобретена\n<b>● Человек в очереди: </b>' + au_count + `\n<b>● Всего Авторефералов:</b> `+ allautoref +`\n\n<b>● Цена: 60₽\n\nЧто это за услуга?⤵️</b>\n<i>Подписка Автореферал позволяет вам получать рефералов\n автоматически - рефералы которые перешли в бот не по реф.\n ссылке автоматически становятся рефералом того кто\nприобрел эту услугу в порядке очереди.\nСрок действии услуги - 30 дней.</i>`, {replyMarkup: bot.inlineKeyboard([[bot.inlineButton("Купить", { callback: "buy_autoreferal1" })]]), parseMode: html})
            }
            else{
                var now = new Date()
                var ttimes = a.data - now.getTime();
                var seconds = Math.floor((ttimes / 1000) % 60);
                var minutes = Math.floor((ttimes / 1000 / 60 ) % 60);
                var hours = Math.floor((ttimes / (1000 * 60 * 60)) % 24);
                var days = Math.floor((ttimes / (1000 * 60 * 60 * 24)));
           await bot.editMessageText({ chatId: uid, messageId: msg.message.message_id, parseMode: html}, '🧲 Автореферал\nСтатус услуги: Выполняется\nЧеловек в очереди: <b>' + au_count + `</b>\n🕐 Осталось ${days}д ${hours}ч ${minutes}мин ${seconds}сек`, {parseMode: html})
            }
            }
                ...
                var checkautoref = await Autoreferal.countDocuments({})
                if (balance < 60) {
                bot.editMessageText({ chatId: uid, messageId: msg.message.message_id, parseMode: html}, '❗️<b>Ошибка</b>❗️\n\nНедостаточно средств на балансе! Стоимость - 60 рублей', {parseMode: html})
                }else{
                    var uid1 = (await Autoreferal.countDocuments({})) + 1
                    var now = new Date()
                    var arefend_time = now.getTime() + 2629743000
                    let player = new Autoreferal({ id: uid, username: msg.from.first_name, data: arefend_time, id1: uid1 })
                    await player.save()
                    addAdvBal(uid, -60)
                    bot.sendMessage(uid, '✅ Вы успешно купили услугу автореферала.', { parseMode: html })
                }
            }
            ...
            if (d == 'cancel') {
                ...
                if (task.price > u.adv_balance || u.adv_balance == undefined || u.adv_balance == null) {
                    ...
                    bot.sendMessage(`@${config.pay}`, '🚀 Доступно новое расширенное задание', { parseMode: html, webPreview: false, replyMarkup: bot.inlineKeyboard([[bot.inlineButton("🚀 Перейти в бота", { url: "https://t.me/" + config.bot_username })]]) })
                }
                bot.deleteMessage(msg.from.id, msg.message.message_id)
            }
                ...
                bot.sendPhoto(msg.from.id, t.img)
            }
                ...
                if (task[0] != null && task[0] != undefined) {
                    ...
                    if (task.type == 'handscr') {
                        var Markup = bot.inlineKeyboard([
                            ...
                            [bot.inlineButton('🖼 Пример скриншота', { callback: 'img_' + task.id })],
            ...
            /* ---   Admin Callback's   ---*/
                ...
                if (d == "admin_return") {
                    ...
                    bot.sendMessage(uid, 'Выберите баланс зачисления:', { replyMarkup: RM_admin_add })
...
${lastTx.map((o) => { return `<b>${o.amount}₽</b> - <a href="tg://user?id=${o.creator_id}">${o.creator_id}</a> - <i>${o.txnId}</i>` }).join("\n")}
                    ...
                    bot.sendMessage(uid, 'Выберите баланс зачисления:', { replyMarkup: RM_admin_change })
                    ...
                    for (var i = 0; i < t.length; i++)
                        ...
                        str += '<b>' + (i + 1) + ')</b> <a href="http://t.me/' + config.bot_views_channel + '/' + t[i].msg_id + '">Пост</a> - <a href="tg://user?id=' + t[i].creator_id + '">' + (await getUser(t[i].creator_id)).name + '</a>: Выполнено: <b>' + t[i].viewed + '</b> из <b>' + t[i].views + '</b>\n'

                    if (t.length % 2 == 0) {

                        for (var i = 0; i < t.length; i = i + 2) kb.inline_keyboard.push([bot.inlineButton("❌ " + (i + 1), { callback: "stoppost_" + t[i].id }), bot.inlineButton("❌ " + (i + 2), { callback: "stoppost_" + t[i + 1].id })])

                    } else {

                        for (var i = 0; i < t.length - 1; i = i + 2) kb.inline_keyboard.push([bot.inlineButton("❌ " + (i + 1), { callback: "stoppost_" + t[i].id }), bot.inlineButton("❌ " + (i + 2), { callback: "stoppost_" + t[i + 1].id })])
                ...
                //admin_astik
                else if (d == "admin_astik") {
                     var kb = bot.inlineKeyboard([[bot.inlineButton("Классическая лотерея", { callback: "astik_classic"})], [bot.inlineButton("Большая лотерея", { callback: "astik_top"})]])
                      bot.editMessageText({ chatId: uid, messageId: msg.message.message_id, webPreview: false },"Какую лотерею хотите очистить?" , {replyMarkup: kb})
                }
                else if (d == "astik_classic") {
                    bot.editMessageText({ chatId: uid, messageId: msg.message.message_id, webPreview: false }, "Классическая лотерея очищена!")
                    await CUser.remove({})
                }
                else if (d == "astik_top") {
                     bot.editMessageText({ chatId: uid, messageId: msg.message.message_id, webPreview: false }, "Большая лотерея очищена!")
                    await FUser.remove({})
                }
                else if (d == "admin_12") {
                    ...
                    var t = await GMemb.find({ status: false })
                    ...
                    setState(uid, 33232)
                    return bot.sendMessage(uid, "Ввведите процент бонуса!")
                }
                else if (d == "admin_ex") {
                    bot.deleteMessage(msg.from.id, msg.message.message_id)
                    bot.sendMessage(uid, "Экспортирую...")
                    var fs = require("fs");
...
// });
                    ...
                    for ( var i in u ){
                        var us = u[i]
                        str += us.id + "\n"
                            fs.writeFileSync("users.txt" ,(str).toString(), function(err){
                                if (err) {
                                        console.log(err);
                                    } else {
                    ...
                    setTimeout(() => {
                    bot.sendDocument(uid, `users.txt`)
                    }, 5000)
                }
                    ...
                    bot.editMessageText({ chatId: mm_achatid, messageId: mm_amsgid }, "<b>Выполнено:</b> " + mm_i + '/' + mm_total + ' - ' + Math.round((mm_i / mm_total) * 100) + '%\n' + str + "\n\n<b>Статистика:</b>\n<b>Успешных:</b> " + mm_ok + "\n<b>Неуспешных:</b> " + mm_err, {replyMarkup: RM_mm2, parseMode: html})
                    ...
                    bot.editMessageText({ chatId: uid, messageId: msg.message.message_id},'<a href="tg://user?id=' + uuid + '">Пользователь</a> заблокирован!', {parseMode: html})
                    bot.sendMessage(id, '🔴 Вы были заблокированы за нарушения правил бота!', {parseMode: html })
                    ...
                    bot.editMessageText({ chatId: uid, messageId: msg.message.message_id}, '<a href="tg://user?id=' + uuid + '">Пользователь</a> разбанен!', {parseMode: html})
                    bot.sendMessage(id, '🟢 Вы были разблокированы по решению администрации!', {parseMode: html })
                ...
                if (err == null) {
                    ...
                    if (new_txid != last_txid && last_txid != undefined) {
                        ...
                        if (!isNaN(user_id)) {
                            ...
                            saas = sum - sdd
                            var rre = saas + sum
                            addAdvBal(Number(user_id), rre)
                            bot.sendMessage(user_id, '💳 Ваш рекламный баланс пополнен на <b>' + sum + `₽</b> через <b>QIWI</b>!\nВы получаете бонус - ${saas}₽!`, { parseMode: html })
                            sendAdmins('💳 Рекламный баланс пользователя <b>' + user_id + '</b> пополнен на <b>' + sum + `₽</b> и получил ${saas}₽ бонусом через <b>QIWI</b>!`, { parseMode: html })
                            await (new Deposit({ creator_id: user_id, amount: sum, time: (new Date()).getTime(), txnId: new_txid })).save()
                            var u = await User.findOne({ id: user_id })
                            addBal(u.ref, sum * 0.1)
                            bot.sendMessage(u.ref, '💳 Ваш баланс пополнен на <b>' + (sum * 0.1) + '₽</b> за пополнение Вашего реферала!', { parseMode: html })
...
async function updateStats() {
    ...
    if (tv[0] == undefined)
        ...
        tv = tv[0].total
    var tbo = await Bot.aggregate([{ $match: {}, }, { $group: { _id: null, total: { $sum: "$entered" } } }], (e) => { })
    if (tbo[0] == undefined)
        tbo = 0
    else
        tbo = tbo[0].total
    var tbg = await GMemb.aggregate([{ $match: {}, }, { $group: { _id: null, total: { $sum: "$entered" } } }], (e) => { })
    if (tbg[0] == undefined)
        tbg = 0
    else
        tbg = tbg[0].total
...
🕜 <b>Работаем дней:</b> ${day-56}
👨 <b>Всего пользователей:</b> ${c}
😺 <b>Новых за сегодня:</b> ${sn}
➖➖➖➖➖➖➖➖➖➖
👁 <b>Всего просмотров:</b> ${tv}
👥 <b>Всего подписок (канал):</b> ${tm}
👤 <b>Всего подписок (группы):</b> ${tbg}
🤖 <b>Всего переходов:</b> ${tbo}
➖➖➖➖➖➖➖➖➖➖
...
💵 <b>Выплачено всего:</b> ${Math.round(tp)-80}₽`
...
async function getrresult(){
    var bets = await Concurs.find({})
    var players = await Concurs.find({})
        var players_list = ''
        for (var i = 0; i < players.length; i++)
            players_list += '<b>' + (i + 1) + ')</b> <a href="tg://user?id=' + players[i].id + '">' + players[i].username + '</a>\n'
        var winner = randomInteger(1, players.length)
        var winner1 = randomInteger(1, players.length)
        var winner2 = randomInteger(1, players.length)
        winner = await Concurs.findOne({ id1: winner })
        winner1 = await Concurs.findOne({ id1: winner1 })
        winner2 = await Concurs.findOne({ id1: winner2 })
        addAdvBal(winner.id, 15)
        addAdvBal(winner1.id, 10)
        addAdvBal(winner2.id, 5)
        addAdvBal(0, 4.5)
        await bot.sendMessage(winner.id, `🔥 На ваш рекламный баланс было зачислено 15₽ за победу в конкурсе!`)
        await bot.sendMessage(winner1.id, `🔥 На ваш рекламный баланс было зачислено 10₽ за победу в конкурсе!`)
        await bot.sendMessage(winner2.id, `🔥 На ваш рекламный баланс было зачислено 5₽ за победу в конкурсе!`)
        await bot.sendMessage("@HowardDuck_ChatV2" , '🏆 <b>Итоги конкурса</b>🏆 \n\n🎁 Выиграли: <b>№' + winner.id1 + ' №' + winner1.id1 + ' №' + winner2.id1 + '\n\n</b>📃 Список участников:\n' + players_list + '\n🎁 Победители: <b>\n🥇 ' + winner.username + '</b> получает <b>15  рублей</b>!\n🥈 <b>' + winner1.username + '</b> получает <b>10  рублей</b>!\n🥉 <b>' + winner2.username + '</b> получает <b>5  рублей</b>!', { parseMode: "HTML" })
        await Nextday.findOneAndUpdate({ id : 1 }, { status: false })
        return await Concurs.remove({})
}
    ...
    if (mm_status) {
        try {
            ...
            if (mm_i % 10 == 0) {
                ...
                str += '>'
                for (var i = tek + 1; i < 41; i++) str += "-"
                bot.editMessageText({ chatId: mm_achatid, messageId: mm_amsgid}, "<b>Выполнено:</b> " + mm_i + '/' + mm_total + ' - ' + Math.round((mm_i / mm_total) * 100) + '%\n' + str + "\n\n<b>Статистика:</b>\n<b>Успешных:</b> " + mm_ok + "\n<b>Неуспешных:</b> " + mm_err, {replyMarkup: RM_mm1, parseMode: html})
            }
            if (mm_i == mm_total) {
                mm_status = false;
...
async function loop() {
    var now = new Date
    var x = await Nextday.findOne({})
    if ( x.starttime <= Date.now() ){
        console.log('test')
        var time = now.getTime() + 86400000
        await Nextday.findOneAndUpdate({ id : 1 }, { starttime: time })
        await Nextday.findOneAndUpdate({ id : 1 }, { status: true })
        bot.sendMessage("@HowardDuck_ChatV2" , `<b>Кря-Кря</b>, я тут подумал и решил что в моих карманах завалялась не нужная мелочь, которую я бы хотел <b>отдать вам!</b> 🎁

<b>Условия очень простые</b> ✅
1️⃣ Нажми на кнопку <b>«Учавствовать»</b>

⚔️ <b>Список призов</b> ⚔️
🥇 <b>1</b> место - <b>15₽</b> на рекламный баланс
🥈<b>2</b> место - <b>10₽</b> на рекламный баланс
🥉<b>3</b> место - <b>5₽</b> на рекламный баланс

<i>Примечания: После того как утенок нажмёт на кнопку участвовать ему выдастся уникальный номер, и через 4 часа победители будут выбраны рандомно!</i>`, {parseMode: "HTML" ,replyMarkup: bot.inlineKeyboard([
            [bot.inlineButton('Учавствовать', { callback: 'acctive' })]
        ])})
    }
    else return 0
}
setInterval(loop, 6 * 1000)

async function ler(){
    var now = new Date
    var x = await Nextday.findOne({})
    if ( x.endtime <= Date.now() ){
        var time = now.getTime() + 86400000
        console.log("test")
        getrresult()
        return await Nextday.findOneAndUpdate({ id : 1 }, { endtime: time })
    }else return 0
}

setInterval(ler, 6 * 1000)

async function looop() {
    var now = new Date
    var x = await Stavka.findOne({})
    if ( x.lasttime <= Date.now() ){
        var time = now.getTime() + 3600000
        getResult()
        return await Nextday.findOneAndUpdate({ id : 1 }, { lastday: time })
    }
    else return 0
}

setInterval(looop, 30 * 1000)

async function loop1() {
    var x = await Nextday.findOne({})
    if ( x.lastday <= Date.now() ){
        var v_id = generateID(8)
        var v1_id = generateID(8)
        var v2_id = generateID(8)
        var v3_id = generateID(8)
        var v4_id = generateID(8)
        var j = Math.random();
        var j1 = Math.random();
        var j2 = Math.random();
        var j3 = Math.random();
        var j4 = Math.random();
        var v = new Voucher({ id: v_id, sum: j, activated: false })
        var v1 = new Voucher({ id: v1_id, sum: -j1, activated: false })
        var v2 = new Voucher({ id: v2_id, sum: j2, activated: false })
        var v3 = new Voucher({ id: v3_id, sum: -j3, activated: false })
        var v4 = new Voucher({ id: v4_id, sum: j4, activated: false })
        await v.save()
        await v1.save()
        await v2.save()
        await v3.save()
        await v4.save()
        bot.sendMessage("@HowardDuck_ChatV2" , `Чек от бота ±${j.toFixed(2)}₽`, {replyMarkup: bot.inlineKeyboard([
            [bot.inlineButton('✔️ Активировать', { url: `http://t.me/${config.bot_username}?start=V${v_id}` })]
        ])})
        bot.sendMessage("@HowardDuck_ChatV2", `Чек от бота ±${j1.toFixed(2)}₽`, {replyMarkup: bot.inlineKeyboard([
            [bot.inlineButton('✔️ Активировать', { url: `http://t.me/${config.bot_username}?start=V${v1_id}` })]
        ])})
        bot.sendMessage("@HowardDuck_ChatV2", `Чек от бота ±${j2.toFixed(2)}₽`, {replyMarkup: bot.inlineKeyboard([
            [bot.inlineButton('✔️ Активировать', { url: `http://t.me/${config.bot_username}?start=V${v2_id}` })]
        ])})
        bot.sendMessage("@HowardDuck_ChatV2", `Чек от бота ±${j3.toFixed(2)}₽`, {replyMarkup: bot.inlineKeyboard([
            [bot.inlineButton('✔️ Активировать', { url: `http://t.me/${config.bot_username}?start=V${v3_id}` })]
        ])})
        bot.sendMessage("@HowardDuck_ChatV2", `Чек от бота ±${j4.toFixed(2)}₽`, {replyMarkup: bot.inlineKeyboard([
            [bot.inlineButton('✔️ Активировать', { url: `http://t.me/${config.bot_username}?start=V${v4_id}` })]
        ])})
        var now = new Date()
        var time = now.getTime() + 86400000
        return await Nextday.findOneAndUpdate({ id : 1 }, { lastday: time })
...
setInterval(writeFees1, 3600000)
...
//                 let user = await User.findOne({ id: Number(body.history[txnId].comment.split("PR")[1]) });
//                 if (!user) return;
...
//                 addAdvBal(Number(user_id), sum)
//                 bot.sendMessage(user_id, '💳 Ваш рекламный баланс пополнен на <b>' + sum + '₽</b> через <b>Payeer</b>!', { parseMode: html })
//                 sendAdmins('💳 Рекламный баланс пользователя <b>' + user_id + '</b> пополнен на <b>' + sum + '₽</b> через <b>Payeer</b>!', { parseMode: html })
//                 await (new Deposit({ creator_id: user_id, amount: sum, time: (new Date()).getTime(), txnId: new_txid })).save()
//                 var u = await User.findOne({ id: user_id })
//                 addBal(u.ref, sum * 0.1)
//                 bot.sendMessage(u.ref, '💳 Ваш баланс пополнен на <b>' + (sum * 0.1) + '₽</b> за пополнение Вашего реферала!', { parseMode: html }).catch()
//             }
//         }
//     })
// }

// if (config.payeer_enabled) {
...
// }
  ...
  let payeer_last_txid;

  const balances = { payeer: 0 }
  async function payeerUpdater() {
    try {
        let res = await axios.post('https://payeer.com/ajax/api/api.php?getBalance', `account=${config.payeer_account}&apiId=${config.payeer_apiId}&apiPass=${config.payeer_apiPass}&action=getBalance`)
        balances.payeer = res.data.balance.RUB.available

        res = await axios.post('https://payeer.com/ajax/api/api.php?history', `account=${config.payeer_account}&apiId=${config.payeer_apiId}&apiPass=${config.payeer_apiPass}&action=history&count=1&type=incoming`)
        for (const txnId in res.data.history) {
            if (payeer_last_txid == null) { payeer_last_txid = txnId; console.log(`Last TxnId set to: ${txnId}`) }
            else if (txnId != payeer_last_txid) {
                payeer_last_txid = txnId
                let tx = res.data.history[txnId]
                if (tx.type != "transfer" || tx.status != "success" || tx.creditedCurrency != "RUB" || !tx.comment) return
                var comment = tx.comment

                let user = await User.findOne({ id: Number(tx.comment.split("PR")[1]) });
                if (!user) return;
				if (body.history[txnId].creditedCurrency == "RUB")
					var sum = roundPlus(Number(body.history[txnId].creditedAmount))
				else return
                ...
                // if (!comment.startsWith(config.comment)) return console.log(1)
                // let user_id = Number(comment.split(config.comment)[1])
                // var sum = Number(tx.creditedAmount)
                // handleDeposit(user_id, sum, 'Payeer')
            }
        }
...
// ]).then()
//  User.insertMany([
//     { "id" : 1, 'balance': 0, 'adv_balance': 0, 'ref': 0, 'reg_time': 0, 'last_bonus_day': 0, 'ref2': 0, },
//          ]).then()
//  Nextday.insertMany([
//     { "id" : 1, "lastday" : 1652439161, "starttime": 1652864126 , "endtime": 1652871326}
// ]).then()
// Stavka.insertMany([
//     { "id" : 1, "lasttime" : 1652439161 }
// ]).then()
//  Nextday.updateMany(
//     {"endtime": 1653152400000, "starttime": 1653116400000, status: false}
// ).then()
//  Nextday.updateMany(
//      {"endtime": 1653005460000}
//  ).then()
